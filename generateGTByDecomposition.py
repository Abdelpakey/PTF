...
if __name__ == '__main__':
    ...
    print 'update_init_corners: ', update_init_corners
    ...
    if initialize_with_rect:
        init_corners = getRectangularApproximation(init_corners)

    # (init_corners_norm, init_norm_mat_inv) = shiftGeometricCenterToOrigin(init_corners)
    ...
    # print 'init_corners:\n', init_corners
    # print 'init_corners_norm:\n', init_corners_norm
    # print 'init_norm_mat_inv:\n', init_norm_mat_inv
    # print 'init_mean_dist: ', init_mean_dist
    ...
    writeCorners(hom_file, init_corners)
    writeCorners(affine_file, init_corners)
    writeCorners(se2_file, init_corners)
    writeCorners(rt_file, init_corners)
    writeCorners(trans_file, init_corners)
    ...
    # # init_pts_norm = util.dehomogenize(init_hom_mat * util.homogenize(std_pts))
    # # init_pts = util.dehomogenize(init_norm_mat_inv * util.homogenize(init_pts_norm))
    #
    #
    # if use_affine_ls:
    # init_affine_mat = computeAffineLS(std_corners, init_corners_norm)
    # else:
    #     if use_inverse_decomposition:
    #         (init_affine_mat, init_proj_mat, init_hom_rec_mat) = decomposeHomographyInverse(init_hom_mat)
    #     else:
    #         (init_affine_mat, init_proj_mat, init_hom_rec_mat) = decomposeHomographyForward(init_hom_mat)
    # print 'init_affine_mat:\n', init_affine_mat
    # if use_inverse_decomposition:
    #     (init_trans_mat, init_rt_mat, init_se2_mat, init_affine_rec_mat, init_affine_params) = decomposeAffineInverse(
    #         init_affine_mat)
    # else:
    #     (init_trans_mat, init_rt_mat, init_se2_mat, init_affine_rec_mat, init_affine_params) = decomposeAffineForward(
    #         init_affine_mat)

    if use_jaccard_error:
        # init_img = getBinaryPtsImage(img_shape, init_pts)
        init_img = getBinaryPtsImage2(img_shape, init_corners)
        cv2.imshow(curr_bin_window_name, init_img)
    # np.savetxt('std_pts.txt', std_pts.transpose(), fmt='%10.6f', delimiter='\t')
    # np.savetxt('init_pts.txt', init_pts.transpose(), fmt='%10.6f', delimiter='\t')
    ...
    prev_corners_norm = init_corners_norm.copy()
    ...
    for i in xrange(1, no_of_frames):
        ...
        if use_affine_ls:
            affine_mat = computeAffineLS(init_corners_norm, curr_corners_norm)
        ...
        # se2_params = getAugmentingSE2Params(init_affine_params[0:4], affine_params[0:4])
        ...
        corner_diff = curr_corners_norm - init_corners_norm
        tx = np.mean(corner_diff[0, :])
        ty = np.mean(corner_diff[1, :])
        trans_mean_mat = getTranslationMatrix(tx, ty)

        hom_corners = util.dehomogenize(curr_norm_mat_inv * hom_mat * init_corners_hm)
        affine_corners = util.dehomogenize(curr_norm_mat_inv * affine_mat * init_corners_hm)
        se2_corners = util.dehomogenize(curr_norm_mat_inv * se2_mat * init_corners_hm)
        rt_corners = util.dehomogenize(curr_norm_mat_inv * rt_mat * init_corners_hm)
        trans_corners = util.dehomogenize(curr_norm_mat_inv * trans_mat * init_corners_hm)
        trans_mean_corners = util.dehomogenize(curr_norm_mat_inv * trans_mean_mat * init_corners_hm)
        ...
        if use_jaccard_error:
            ...
            # se2_pts = util.dehomogenize(curr_norm_mat_inv * se2_mat * util.homogenize(init_pts_norm))
            ...
            # rt_pts = util.dehomogenize(curr_norm_mat_inv * rt_mat * util.homogenize(init_pts_norm))
            ...
            # trans_pts = util.dehomogenize(curr_norm_mat_inv * trans_mat * util.homogenize(init_pts_norm))
            ...
            # trans_mean_pts = util.dehomogenize(curr_norm_mat_inv * trans_mean_mat * util.homogenize(init_pts_norm))
        ...
        if update_init_corners:
            ...
            init_corners_hm = util.homogenize(init_corners_norm)
