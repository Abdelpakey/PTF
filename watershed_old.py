...
def smallest_neighbor_idx(img, row, col, visited, size = 3):
    _min_row = []
    _min_col = []
    min_val = np.inf
    h, w = img.shape
    win_range = int(size/2)
...
def getRegionalMinima(img, size=3):
    ...
    for row in xrange(h):
        for col in xrange(w):
            # visited[row, col] = 1
            min_row, min_col = smallest_neighbor_idx(img, row, col, visited, size)
            if not min_row or not min_col:
                raise LookupError('getRegionalMinima :: Nearest neighbor for ({:d}, {:d}) not found'.format(row, col))
...
def find_label(img, markers, row, col, visited):
    min_row, min_col = smallest_neighbor_idx(img, row, col, visited)
    print 'find_label :: markers:\n', markers
    ...
    min_row = min_row[0]
    min_col = min_col[0]
    markers[min_row, min_col] = find_label(img, markers, min_row, min_col, visited)
    visited[min_row, min_col] = 1
    return markers[min_row, min_col]


def recursive(img, markers):
    print 'Running recursive watershed'
    visited = np.zeros(img.shape)
    h, w = img.shape
    for row in xrange(h):
        for col in xrange(w):
            if markers[row, col] > 0:
                continue
            visited[row, col] = 1
            markers[row, col] = find_label(img, markers, row, col, visited)
    return markers


def iterative_check_min(img, markers, size = 3):
    print 'Running iterative watershed with the minimum neighbor being checked'
